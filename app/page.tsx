...
import { getServerSession } from "next-auth"; // 세션 가져오는 함수
import { authOptions } from "@/lib/authOptions";

export default async function Home() {
  const session = await getServerSession(authOptions); // 로그인 정보
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      <section className="w-full py-24 lg:py-32 flex flex-col items-center text-center px-4 animate-in fade-in zoom-in duration-500">
        ...
        <p className="text-xl text-muted-foreground mb-10 max-w-2xl leading-relaxed">
          내 진짜 이메일은 숨기고, 
          <span className="font-semibold text-foreground"> @manito.kr </span>
          가상 주소를 사용해 보세요!<br />
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
          ...
          {/* <Button variant="outline" size="lg" className="text-lg h-12 px-8">
            기능 더 알아보기 <ArrowRight className="ml-2 h-5 w-5" />
          </Button> */}
        </div>
      </section>
      ...
      <section className="w-full max-w-6xl px-4 py-20 grid md:grid-cols-3 gap-6">
        ...
        <Card className="border-2 hover:border-primary/50 transition-colors shadow-sm">
          <CardHeader>
            ...
            <CardTitle className="text-xl">이메일 자동 분류</CardTitle>
          </CardHeader>
          <CardContent className="text-muted-foreground">
        ...
        <Card className="border-2 hover:border-primary/50 transition-colors shadow-sm">
          <CardHeader>
            ...
            <CardTitle className="text-xl">디지털 탄소 절감</CardTitle>
          </CardHeader>
          <CardContent className="text-muted-foreground">
